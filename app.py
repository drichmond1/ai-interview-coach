...
# Load API key from environment — hidden from UI when set
...
with gr.Blocks(title="AI Interview Coach") as demo:
    ...
    def handle_start(jd, provider, api_key):
        ...
        yield (new_state, gr.update(value="", visible=False),
    ...
    def handle_submit(current_state, answer, chat_history):
        if not answer.strip():
            yield (current_state, gr.update(value="❌ Please type an answer before submitting.", visible=True),
        ...
        history.append(_msg("assistant", feedback))

        if new_state.phase == "done":
            history.append(_msg("assistant", next_or_summary))
            yield (new_state, gr.update(value="", visible=False), history,
    ...
    def handle_end(current_state, chat_history):
        ...
        return (current_state, gr.update(value="", visible=False), history,
                gr.update(visible=False), gr.update(visible=True))
